...
- **Billing Cycles**:
  ...
  - `weekly` $\rightarrow \text{price} \times \frac{52}{12}$ ($\approx 4.333$ weeks/month)
  - `daily` $\rightarrow \text{price} \times \frac{365}{12}$ ($\approx 30.416$ days/month)
...
- Evaluates due-soon subscriptions ($t \le 7\text{ days}$), urgent cancellation reminders, and student discount expirations ($t \le 30\text{ days}$).
