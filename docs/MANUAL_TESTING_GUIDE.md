...
- **Steps**:
  ...
  4. Select Tier: **"Tier 1 — ₱100.00 / mo"**.
  5. Click **"Create Subscription"**.
- **Expected Result**:
  - Name automatically populates as `Twitch - Keipup (Tier 1)`.
  - Price is `100.00 PHP`, cycle is `Monthly`, payment method is `GCash`.
  - In the table, the Twitch row displays the vibrant purple Twitch SVG icon (`/icons/twitch_colored.svg`).
...
- **Steps**:
  ...
  3. Select Tier: **"Nitro Full — ₱263.99 / mo"**.
  4. Click **"Create Subscription"**.
- **Expected Result**:
  - Subscription appears in the table with the blurple Discord SVG icon (`/icons/discord_colored.svg`).
  - Monthly Spend KPI increases by `₱263.99`.
...
- **Steps**:
  1. Add a personal subscription (e.g. `Spotify` at `₱149.00/mo`, `Paid by me = checked`).
  2. Add a shared subscription (e.g. `Netflix Family` at `₱549.00/mo`, `Paid by me = unchecked`).
  3. Inspect the **Category Breakdown** card:
     - Click **"Paid by Me"**: Total displays `₱149.00/mo` and shows only personal subscriptions.
     - Click **"Shared"**: Total displays `₱549.00/mo` and shows only shared/covered subscriptions.
     - Click **"Overall"**: Total displays `₱698.00/mo` and shows the combined ecosystem value.
- **Expected Result**:
...
- **Steps**:
  ...
  2. Enter:
     ...
     - **Next Renewal Date**: _(6 months from today)_
     - **Category**: `Cloud & Dev`
     - **Plan Structure**: `Solo`
...
- **Expected Result**:
  - Cost is normalized to monthly equivalent in PHP:
    $$\text{Monthly} = \frac{\$100 \times 58.50\text{ PHP}}{12} = ₱487.50$$
  - Monthly Spend KPI increases by `₱487.50`.
...
- **Expected Result**:
  - A rose/red alert banner appears: `"Cancel Before Renewal: ..."`.
